...
$(document).ready(function() {
  let x = 1;
  let addHasilButton = $('#add-hasil-button');
  let addHasilWrapper = $('#add-hasil-wrapper');
  let autoHasil = "";

  $(addHasilButton).click(function(){
    if($('#add-hasil-pekerjaan').val()) {
      let hasil = (autoHasil == "") ? $('#add-hasil-pekerjaan').val() : autoHasil;
      let pisahHasil = hasil.split(".");
      let pisahHasil2 = pisahHasil[1].split("/");
      autoHasil = pisahHasil[0] + '.' + ++pisahHasil2[0] + '/' + pisahHasil2[1];
    }
    let addHasilHTML = '<div class="form-group"><label>Hasil Pekerjaan Yang Diterima ' + ++x + ': <code>*</code></label> <button type="button" class="btn btn-danger btn-xs" id="remove-hasil-button">&nbsp;<i class="nav-icon fas fa-minus"></i>&nbsp;</button><input type="text" class="form-control" name="hasil_pekerjaan_yang_diterima[]" required placeholder="Masukkan hasil pekerjaan yang diterima" value="' + autoHasil + '"></div>';
    $(addHasilWrapper).append(addHasilHTML);
  });

  $(addHasilWrapper).on('click', '#remove-hasil-button', function(e){
    $(this).parent('div').remove();
		x--;
  });

  let y = 1;
  let addSertipikatButton = $('#add-sertipikat-button');
  let addSertipikatWrapper = $('#add-sertipikat-wrapper');
  let autoSeri = "";

  $(addSertipikatButton).click(function(){
    let seri = (autoSeri == "") ? $('#add-no-seri-sertipikat').val() : autoSeri;
    autoSeri = incrementString(seri);
    let addSertipikatHTML = '<div class="form-group"><label>No. Seri Sertipikat ' + ++y + ': <code>*</code></label> <button type="button" class="btn btn-danger btn-xs" id="remove-hasil-button">&nbsp;<i class="nav-icon fas fa-minus"></i>&nbsp;</button><input type="text" class="form-control" name="no_seri_sertipikat[]" required placeholder="Masukkan no. seri sertipikat" value="' + autoSeri + '"></div>';
    $(addSertipikatWrapper).append(addSertipikatHTML);
  });

  $(addSertipikatWrapper).on('click', '#remove-hasil-button', function(e){
      $(this).parent('div').remove();
      y--;
  });
});
